...
{
    std::string geompath = GeometryPath;
    std::ifstream vGeomFile;

    vGeomFile.open(geompath);

    if (!vGeomFile)
        mLogger->logText("ERROR SHADER FILE " + std::string(geompath) + " NOT SUCCESFULLY READ", LogType::REALERROR);

    std::stringstream vGeomStream;
    vGeomStream << vGeomFile.rdbuf();
    vGeomFile.close();

    std::string geomCode = vGeomStream.str();

    const GLchar* vGeomCode = geomCode.c_str();

    GLuint geometry;

    geometry = glCreateShader(GL_GEOMETRY_SHADER);

    glShaderSource(geometry, 1, &vGeomCode, nullptr);
